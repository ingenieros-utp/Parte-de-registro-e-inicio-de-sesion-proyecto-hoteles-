...
int total_precio=0;
string usuario_i;
unsigned long long costo_total;
...
void confirmar(string &nombre_hotel);
int cerrar();
void ver_reserva();
void ver_reserva_admin();
void eliminar_reserva(string &nombre_hotel);
    ...
    if(n_clien == 0){//no hay datos de usuarios en la estructuras
		...
		if( i ==  n_clien){//si no existe el usuario
			...
			for(int i=0; i < n_clien;i++){
    			...
    			if(strcmp(usuario1.c_str(),usuarios[i].usuario)==0){
    				system("cls");
...
void inic_sesion(int &clien, int &n_clien, bool &sesion, string &nombre_hotel){
	...
	for(int i = 0; i < n_clien;i++){
		...
		if((strcmp(usua.c_str(),usuarios[i].usuario)==0) && (strcmp(clave.c_str(),usuarios[i].contrasena)==0)){
			...
			strcpy(arch_reserva.usuario,usuarios[i].usuario);
			usuario_i = usuarios[i].usuario;
			break;
		}else{
...
void menu_admin(bool &sesion,int &clien,int &n_clien){
	...
	do{
		do{
			...
			cout<<"4. Para ver las reservas.\n";
			cout<<"5. Para agregar hoteles.\n";
...
void menu_clien(bool &sesion, string &nombre_hotel){
	...
	do{
		do{
	    	...
	    	cout<<"3. Si desea cambiar las fechas."<<endl;
	    	cout<<"4. Si desea cambiar las estrellas y valoracion."<<endl;
	    	cout<<"5. Si desea cambiar los filtros."<<endl;
	    	cout<<"6. Si desea ver los comentarios del hotel que escogio."<<endl;
	    	cout<<"7. Si desea ingresar un comentario."<<endl;
	    	cout<<"8. Si desea ver el costo total."<<endl;
	    	cout<<"9. Si desea confirmar la reserva o volver al inicio."<<endl;
	    	cout<<"10. Si desea ver las reservas que ha realizado."<<endl;
	    	cout<<"11. Si desea eliminar alguna reserva."<<endl;
			cout<<"Que opcion desea?: ";
			cin>>op;
			fflush(stdin);
		} while ((op>11) || (op<0));
...
void mostrarhoteles(ciudades &ciudad, string &nombre_hotel){
    ...
    while(fread(&ciudad,sizeof(ciudad),1,archdisco)==1){
        ...
        if ((reserva.hotel.ciudad == ciudad.ciudad) && (reserva.hotel.estrellas <= ciudad.estrellas) &&
            ...
            if (ciudad.piscina == 1){
            	cout<<"Piscina?: Disponible";
			}
			else{
				cout<<"Piscina?: No disponible";
			}
            fflush(stdin);

            if (ciudad.parqueadero == 1){
            	cout<<"Parqueadero?: Disponible";
			}
			else{
				cout<<"Parqueadero?: No disponible";
			}
            fflush(stdin);

            if (ciudad.restaurante==1){
            	cout<<"Restaurante?: Disponible";
			}
			else{
				cout<<"Restaurante?: No disponible";
			}
            fflush(stdin);
...
void cambiar_fil(){
    ...
    cout<<"\nA continuacion tendremos algunos filtros, ingrese 1 si desea agregarlos o 0 si no los requiere.\n";
    cout<<"Piscina: "; cin>>reserva.filtros.piscina; fflush(stdin);
    cout<<"Parqueadero: "; cin>>reserva.filtros.parqueadero; fflush(stdin);
    cout<<"Restaurante: "; cin>>reserva.filtros.restaurante; fflush(stdin);
    
    system("cls");
    cout<<"==Filtros Modificados=="<<endl;
    system("pause");
}

//-------------------------------------------------------------------------------------------------------------------------

//Funcion para cambiar la valoracion y las estrellas
...
void camb_val_estr(){
    ...
    system("cls");
    cout<<"==Valoracion y estrellas cambiadas=="<<endl;
    system("pause");
}
...
void cambiar_fechas(){
    ...
    cout<<"\nIngrese la fecha de llegada.\n";
    cout<<"Dia: "; cin>>reserva.llegada.dia;fflush(stdin);
    cout<<"Mes: "; cin>>reserva.llegada.mes;fflush(stdin);
    cout<<"Año: "; cin>>reserva.llegada.anio;fflush(stdin);
    cout<<"Hora: "; cin>>reserva.llegada.hora;fflush(stdin);
    
    cout<<"\nIngrese la fecha de salida.\n";
	cout<<"Dia: "; cin>>reserva.salida.dia;fflush(stdin);
    cout<<"Mes: "; cin>>reserva.salida.mes;fflush(stdin);
    cout<<"Año: "; cin>>reserva.salida.anio;fflush(stdin);
     cout<<"Hora: "; cin>>reserva.salida.hora;fflush(stdin);
               
    system("cls");
    cout<<"==Fechas de llegada y salida modificadas=="<<endl;
    system("pause");
}
//_________________________________________________________________________________________________________________________
...
void precio_total(ciudades &ciudad, string &nombre_hotel){
	...
	int hora = reserva.llegada.hora;
	int dia = reserva.llegada.dia;
	int mes = reserva.llegada.mes;
	int anio = reserva.llegada.anio;
	...
	else if (((reserva.salida.mes*30+reserva.salida.dia)-(mes1_1*30+reserva.llegada.dia) >= 30) || 
	(mes1_1 > reserva.salida.mes)){
    ...
    while (fread(&ciudad,sizeof(ciudad),1,archdisco)==1){
		...
		(reserva.entradas == ciudad.personas)){
    		...
    		total_precio = ciudad.precio * dias + ((ciudad.precio/24)*horas);
		}
		
    }
    
    reserva.llegada.hora = hora;
	reserva.llegada.dia = dia;
	reserva.llegada.mes = mes;
	reserva.llegada.anio = anio;
...
void confirmar(string &nombre_hotel){
	...
	archi1 = fopen("Hoteles.txt","r+");
	archi2 = fopen("Temporal.txt","at+");
	
	while(fread(&ciudad,sizeof(ciudad),1,archi1)==1){
		
		if ((nombre_hotel == ciudad.hotel) && (reserva.entradas == ciudad.personas) &&
		(reserva.hotel.ciudad == ciudad.ciudad)){
			
			ciudad.hab_disp--;
			fwrite(&ciudad,sizeof(ciudad),1,archi2);
		}
		else{
			fwrite(&ciudad,sizeof(ciudad),1,archi2);
		}
	...
	remove("Hoteles.txt");
	rename("temporal.txt","Hoteles.txt");
	//____________________________________________________________________________________________________________
	arch_reserva.usuario;
    
    arch_reserva.reserva.hotel.ciudad = reserva.hotel.ciudad;
    fflush(stdin);
    
    arch_reserva.nombre_hotel = nombre_hotel;
    fflush(stdin);
    
    arch_reserva.hotel.hab_disp = 1;
    fflush(stdin);
    
    arch_reserva.reserva.llegada.hora = reserva.llegada.hora;
    fflush(stdin);
    
    arch_reserva.reserva.llegada.dia = reserva.llegada.dia;
    fflush(stdin);
    
    arch_reserva.reserva.llegada.mes = reserva.llegada.mes;
    fflush(stdin);
    
    arch_reserva.reserva.llegada.anio = reserva.llegada.anio;
    fflush(stdin);
    
    arch_reserva.reserva.salida.hora = reserva.salida.hora;
    fflush(stdin);
    
    arch_reserva.reserva.salida.dia = reserva.salida.dia;
    fflush(stdin);
    
    arch_reserva.reserva.salida.mes = reserva.salida.mes;
    fflush(stdin);
    
    arch_reserva.reserva.salida.anio = reserva.salida.anio;
    fflush(stdin);
    
    arch_reserva.precio_total = total_precio;
    fflush(stdin);
    
    FILE *archdisco;
	archdisco = fopen("Reserva.txt","at+");
    fwrite(&arch_reserva,sizeof(arch_reserva),1,archdisco);
    fclose(archdisco);
	//____________________________________________________________________________________________________________
...
void ver_reserva(){
	...
	system ("cls");
	getchar();
	cout<<"Sus reservas son: "<<endl<<endl;
    FILE *archdisco;
    archdisco = fopen("Reserva.txt","at+");
    
    while (fread(&arch_reserva,sizeof(arch_reserva),1,archdisco)==1){
    	
    	if ((usuario_i == arch_reserva.usuario) && (arch_reserva.hotel.hab_disp > 0)){
    		
    		cout<<"Usuario: "<<arch_reserva.usuario<<endl;
    		fflush(stdin);
    		
    		cout<<"Ciudad: "<<arch_reserva.reserva.hotel.ciudad<<endl;
    		fflush(stdin);
    		
    		cout<<"Hotel: "<<arch_reserva.nombre_hotel<<endl;
    		fflush(stdin);
    		
    		cout<<"Habitaciones reservadas: "<<arch_reserva.hotel.hab_disp<<endl;
    		fflush(stdin);
    		
    		cout<<"Hora de llegada: "<<arch_reserva.reserva.llegada.hora<<endl;
    		fflush(stdin);
    		
    		cout<<"Dia de llegada: "<<arch_reserva.reserva.llegada.dia<<endl;
    		fflush(stdin);
    		
    		cout<<"Mes de llegada: "<<arch_reserva.reserva.llegada.mes<<endl;
    		fflush(stdin);
    		
   			cout<<"Anio de llegada: "<<arch_reserva.reserva.llegada.anio<<endl;
    		fflush(stdin);
    		
    		cout<<"Hora de salida: "<<arch_reserva.reserva.salida.hora<<endl;
    		fflush(stdin);
    		
    		cout<<"Dia de salida: "<<arch_reserva.reserva.salida.dia<<endl;
    		fflush(stdin);
    		
    		cout<<"Mes de salida: "<<arch_reserva.reserva.salida.mes<<endl;
    		fflush(stdin);
    		
   			cout<<"Anio de salida: "<<arch_reserva.reserva.salida.anio<<endl;
    		fflush(stdin);
    		
    		cout<<"Precio total a pagar "<<arch_reserva.precio_total<<endl;
    		fflush(stdin);
...
void ver_reserva_admin(){
	...
	system ("cls");
	getchar();
	cout<<"Las reservas son: "<<endl<<endl;
    FILE *archdisco;
    archdisco = fopen("Reserva.txt","at+");
    
    while (fread(&arch_reserva,sizeof(arch_reserva),1,archdisco)==1){
    	
    	if (arch_reserva.hotel.hab_disp > 0){
...
void eliminar_reserva(string &nombre_hotel){
	...
	cout<<"Digite los datos para la reserva que eliminara: "<<endl;
	cout<<"Digite el nombre del hotel: "; cin>>nombre_hotel;
	cout<<"Digite el total a pagar: "; cin>>costo_total;
	
	FILE *archi1;
	FILE *archi2;
	
	archi1 = fopen("Reservas.txt","r+");
	archi2 = fopen("Temporal.txt","at+");
	
	while(fread(&arch_reserva,sizeof(arch_reserva),1,archi1)==1){
		
		if ((nombre_hotel == arch_reserva.nombre_hotel) && (costo_total == arch_reserva.precio_total)){
			
			arch_reserva.hotel.hab_disp--;
			fwrite(&arch_reserva,sizeof(arch_reserva),1,archi2);
		}
		else{
			fwrite(&arch_reserva,sizeof(arch_reserva),1,archi2);
		}
	...
	remove("Reserva.txt");
	rename("temporal.txt","Reserva.txt");
